    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (btnOrder != null) {
            btnOrder.setOnClickListener(v -> {
                // Для Арбата тоже можно открыть меню или заглушку
    ...
    private void showPreOrderPrompt() {
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("Да, выбрать блюда", (dialog, which) -> {
                    // Пока перекидываем в общее меню пиццы как пример
                    Intent intent = new Intent(Arbatactivity.this, zakazpizzaActivity.class);
